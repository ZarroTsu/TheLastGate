...
int sv_terminology(unsigned char *buf) {
    ...
    if (tn >= 0) {
        ...
        switch (buf[1]) {
            case ST_TREE_ICON: sk_tree[tn][n].icon = *(unsigned short *) (buf + 3);
                return 5;
            case ST_TREE_NAME1: memcpy(sk_tree[tn][n].name, buf + 3, 10);
                return 13;
            case ST_TREE_NAME2: memcpy(sk_tree[tn][n].name + 10, buf + 3, 10);
                return 13;
            case ST_TREE_NAME3: memcpy(sk_tree[tn][n].name + 20, buf + 3, 10);
                return 13;
            case ST_TREE_DESC1A: memcpy(sk_tree[tn][n].dsc1, buf + 3, 10);
                return 13;
            case ST_TREE_DESC1B: memcpy(sk_tree[tn][n].dsc1 + 10, buf + 3, 10);
                return 13;
            case ST_TREE_DESC1C: memcpy(sk_tree[tn][n].dsc1 + 20, buf + 3, 10);
                return 13;
            case ST_TREE_DESC1D: memcpy(sk_tree[tn][n].dsc1 + 30, buf + 3, 10);
                return 13;
            case ST_TREE_DESC1E: memcpy(sk_tree[tn][n].dsc1 + 40, buf + 3, 10);
                return 13;
            case ST_TREE_DESC2A: memcpy(sk_tree[tn][n].dsc2, buf + 3, 10);
                return 13;
            case ST_TREE_DESC2B: memcpy(sk_tree[tn][n].dsc2 + 10, buf + 3, 10);
                return 13;
            case ST_TREE_DESC2C: memcpy(sk_tree[tn][n].dsc2 + 20, buf + 3, 10);
                return 13;
            case ST_TREE_DESC2D: memcpy(sk_tree[tn][n].dsc2 + 30, buf + 3, 10);
                return 13;
            case ST_TREE_DESC2E: memcpy(sk_tree[tn][n].dsc2 + 40, buf + 3, 10);
                return 13;
            default: break;
        }
    }
    ...
    if (buf[0] == SV_TERM_SKILLS) {
        n = buf[2];
        switch (buf[1]) {
            case ST_SKILLS_SORT: raw_skilltab[n].sortkey = *(unsigned char *) (buf + 3);
                raw_skilltab[n].attrib[0] = *(unsigned char *) (buf + 4);
                raw_skilltab[n].attrib[1] = *(unsigned char *) (buf + 5);
                raw_skilltab[n].attrib[2] = *(unsigned char *) (buf + 6);
                raw_skilltab[n].show = *(unsigned char *) (buf + 7);
                return 8;
            case ST_SKILLS_NAME1: memcpy(raw_skilltab[n].name, buf + 3, 10);
                return 13;
            case ST_SKILLS_NAME2: memcpy(raw_skilltab[n].name + 10, buf + 3, 10);
                return 13;
            case ST_SKILLS_NAME3: memcpy(raw_skilltab[n].name + 20, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC01: memcpy(raw_skilltab[n].desc, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC02: memcpy(raw_skilltab[n].desc + 10, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC03: memcpy(raw_skilltab[n].desc + 20, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC04: memcpy(raw_skilltab[n].desc + 30, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC05: memcpy(raw_skilltab[n].desc + 40, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC06: memcpy(raw_skilltab[n].desc + 50, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC07: memcpy(raw_skilltab[n].desc + 60, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC08: memcpy(raw_skilltab[n].desc + 70, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC09: memcpy(raw_skilltab[n].desc + 80, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC10: memcpy(raw_skilltab[n].desc + 90, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC11: memcpy(raw_skilltab[n].desc + 100, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC12: memcpy(raw_skilltab[n].desc + 110, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC13: memcpy(raw_skilltab[n].desc + 120, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC14: memcpy(raw_skilltab[n].desc + 130, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC15: memcpy(raw_skilltab[n].desc + 140, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC16: memcpy(raw_skilltab[n].desc + 150, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC17: memcpy(raw_skilltab[n].desc + 160, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC18: memcpy(raw_skilltab[n].desc + 170, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC19: memcpy(raw_skilltab[n].desc + 180, buf + 3, 10);
                return 13;
            case ST_SKILLS_DESC20: memcpy(raw_skilltab[n].desc + 190, buf + 3, 10);
                return 13;
            default: break;
        }
    }
    ...
    if (buf[0] == SV_TERM_META) {
        ...
        switch (buf[1]) {
            case ST_META_NAME1: memcpy(meta_stats[n].name, buf + 3, 10);
                return 13;
            case ST_META_NAME2: memcpy(meta_stats[n].name + 10, buf + 3, 10);
                return 13;
            case ST_META_NAME3: memcpy(meta_stats[n].name + 20, buf + 3, 10);
                return 13;
            case ST_META_DESC01: memcpy(meta_stats[n].desc, buf + 3, 10);
                return 13;
            case ST_META_DESC02: memcpy(meta_stats[n].desc + 10, buf + 3, 10);
                return 13;
            case ST_META_DESC03: memcpy(meta_stats[n].desc + 20, buf + 3, 10);
                return 13;
            case ST_META_DESC04: memcpy(meta_stats[n].desc + 30, buf + 3, 10);
                return 13;
            case ST_META_DESC05: memcpy(meta_stats[n].desc + 40, buf + 3, 10);
                return 13;
            case ST_META_DESC06: memcpy(meta_stats[n].desc + 50, buf + 3, 10);
                return 13;
            case ST_META_DESC07: memcpy(meta_stats[n].desc + 60, buf + 3, 10);
                return 13;
            case ST_META_DESC08: memcpy(meta_stats[n].desc + 70, buf + 3, 10);
                return 13;
            case ST_META_DESC09: memcpy(meta_stats[n].desc + 80, buf + 3, 10);
                return 13;
            case ST_META_DESC10: memcpy(meta_stats[n].desc + 90, buf + 3, 10);
                return 13;
            case ST_META_DESC11: memcpy(meta_stats[n].desc + 100, buf + 3, 10);
                return 13;
            case ST_META_DESC12: memcpy(meta_stats[n].desc + 110, buf + 3, 10);
                return 13;
            case ST_META_DESC13: memcpy(meta_stats[n].desc + 120, buf + 3, 10);
                return 13;
            case ST_META_DESC14: memcpy(meta_stats[n].desc + 130, buf + 3, 10);
                return 13;
            case ST_META_DESC15: memcpy(meta_stats[n].desc + 140, buf + 3, 10);
                return 13;
            case ST_META_DESC16: memcpy(meta_stats[n].desc + 150, buf + 3, 10);
                return 13;
            case ST_META_DESC17: memcpy(meta_stats[n].desc + 160, buf + 3, 10);
                return 13;
            case ST_META_DESC18: memcpy(meta_stats[n].desc + 170, buf + 3, 10);
                return 13;
            case ST_META_DESC19: memcpy(meta_stats[n].desc + 180, buf + 3, 10);
                return 13;
            case ST_META_DESC20: memcpy(meta_stats[n].desc + 190, buf + 3, 10);
                return 13;
            case ST_META_VALUES:
                meta_stats[n].value = *(unsigned short *) (buf + 3);
...
int sv_setmap(unsigned char *buf, int off) {
    ...
    if (n < 0 || n >= screen_renderdist * screen_renderdist) {
    ...
    if (buf[1] & 64) {
        ...
        p += 2;
        map[n].ch_speed = *(short int *) (buf + p);
        p += 2;
        cnt[6]++;
    }
    if (buf[1] & 128) {
        ...
        map[n].ch_castspd = *(short int *) (buf + p);
        p += 2;
        map[n].ch_atkspd = *(short int *) (buf + p);
        p += 2;
        map[n].ch_movespd = *(short int *) (buf + p);
        p += 2;
        map[n].ch_fontcolor = *(unsigned char *) (buf + p);
        p += 1;
        cnt[7]++;
    }
    return p;
...
void sv_scroll_down(unsigned char *buf) {
    ...
    memmove(map, map + screen_renderdist,
            sizeof(struct cmap) * (screen_renderdist * screen_renderdist - screen_renderdist));
}
...
void sv_scroll_up(unsigned char *buf) {
    ...
    memmove(map + screen_renderdist, map,
            sizeof(struct cmap) * (screen_renderdist * screen_renderdist - screen_renderdist));
}
...
void sv_scroll_leftup(unsigned char *buf) {
    ...
    memmove(map + screen_renderdist + 1, map,
            sizeof(struct cmap) * (screen_renderdist * screen_renderdist - screen_renderdist - 1));
}
...
void sv_scroll_leftdown(unsigned char *buf) {
    ...
    memmove(map, map + screen_renderdist - 1,
            sizeof(struct cmap) * (screen_renderdist * screen_renderdist - screen_renderdist + 1));
}
...
int sv_cmd(unsigned char *buf) {
    ...
    switch (buf[0]) {
        case SV_SETCHAR_NAME1: sv_setchar_name1(buf);
            break;
        case SV_SETCHAR_NAME2: sv_setchar_name2(buf);
            break;
        case SV_SETCHAR_NAME3: sv_setchar_name3(buf);
            break;
        case SV_SETCHAR_MODE: sv_setchar_mode(buf);
            return 2;
        case SV_SETCHAR_ATTRIB: sv_setchar_attrib(buf);
            return 8;
        case SV_SETCHAR_SKILL: sv_setchar_skill(buf);
            return 8;
        case SV_SETCHAR_HP: sv_setchar_hp(buf);
            return 13;
        case SV_SETCHAR_ENDUR: sv_setchar_endur(buf);
            return 13;
        case SV_SETCHAR_MANA: sv_setchar_mana(buf);
            return 13;
        case SV_SETCHAR_AHP: sv_setchar_ahp(buf);
            return 3;
        case SV_SETCHAR_AEND: sv_setchar_aend(buf);
            return 3;
        case SV_SETCHAR_AMANA: sv_setchar_amana(buf);
            return 3;
        case SV_SETCHAR_DIR: sv_setchar_dir(buf);
            return 2;

        case SV_SETCHAR_PTS: sv_setchar_pts(buf);
            return 13;
        case SV_SETCHAR_WPS: sv_setchar_wps(buf);
            return 13;
        case SV_SETCHAR_TOK: sv_setchar_tok(buf);
            return 9;
        case SV_SETCHAR_TRE: sv_setchar_tre(buf);
            return 3;
        case SV_SETCHAR_GOLD: sv_setchar_gold(buf);
            return 13;
        case SV_SETCHAR_ITEM: sv_setchar_item(buf);
            return 11;
        case SV_SETCHAR_WORN: sv_setchar_worn(buf);
            return 10;
        case SV_SETCHAR_SPELL: sv_setchar_spell(buf);
            return 9;
        case SV_SETCHAR_OBJ: sv_setchar_obj(buf);
            return 6;
        case SV_SETCHAR_LOCA1: sv_setchar_location1(buf);
            return 11;
        case SV_SETCHAR_LOCA2: sv_setchar_location2(buf);
            return 11;
        ...
        case SV_SETORIGIN: sv_setorigin(buf);
            return 5;

        case SV_TICK: sv_tick(buf);
            return 2;

        case SV_LOG0: sv_log(buf, 0);
            break;
        case SV_LOG1: sv_log(buf, 1);
            break;
        case SV_LOG2: sv_log(buf, 2);
            break;
        case SV_LOG3: sv_log(buf, 3);
            break;
        case SV_LOG4: sv_log(buf, 4);
            break;
        case SV_LOG5: sv_log(buf, 5);
            break;
        case SV_LOG6: sv_log(buf, 6);
            break;
        case SV_LOG7: sv_log(buf, 7);
            break;
        case SV_LOG8: sv_log(buf, 8);
            break;
        case SV_LOG9: sv_log(buf, 9);
            break;
        ...
        case SV_MOTD0: sv_motd(buf, 0);
            break;
        case SV_MOTD1: sv_motd(buf, 1);
            break;
        case SV_MOTD2: sv_motd(buf, 2);
            break;
        case SV_MOTD3: sv_motd(buf, 3);
            break;

        case SV_SCROLL_RIGHT: sv_scroll_right(buf);
            return 1;
        case SV_SCROLL_LEFT: sv_scroll_left(buf);
            return 1;
        case SV_SCROLL_DOWN: sv_scroll_down(buf);
            return 1;
        case SV_SCROLL_UP: sv_scroll_up(buf);
            return 1;

        case SV_SCROLL_RIGHTDOWN: sv_scroll_rightdown(buf);
            return 1;
        case SV_SCROLL_RIGHTUP: sv_scroll_rightup(buf);
            return 1;
        case SV_SCROLL_LEFTDOWN: sv_scroll_leftdown(buf);
            return 1;
        case SV_SCROLL_LEFTUP: sv_scroll_leftup(buf);
            return 1;

        case SV_LOOK1: sv_look1(buf);
            break;
        case SV_LOOK2: sv_look2(buf);
            break;
        case SV_LOOK3: sv_look3(buf);
            break;
        case SV_LOOK4: sv_look4(buf);
            break;
        case SV_LOOK5: sv_look5(buf);
            break;
        case SV_LOOK6: sv_look6(buf);
            break;
        case SV_LOOK7: sv_look7(buf);
            return 8;
        case SV_LOOK8: sv_look8(buf);
            return 8;

        case SV_CLOSESHOP: sv_closeshop();
            return 1;

        case SV_SETTARGET: sv_settarget(buf);
            return 13;

        case SV_PLAYSOUND: sv_playsound(buf);
            return 13;

        case SV_EXIT: sv_exit(buf);
            break;

        case SV_LOAD: sv_load(buf);
            return 5;

        case SV_UNIQUE: sv_unique(buf);
            return 9;
        case SV_IGNORE: return sv_ignore(buf);

        case SV_WAYPOINTS: sv_waypoints();
            return 1;
        case SV_SHOWMOTD: sv_showmotd(buf);
            return 2;

        case SV_CLEARBOX: sv_clearbox(buf);
            return 9;

        default: xlog(0, "Unknown SV: %d", buf[0]);
            return -1;
    }
...
int tick_do(void) {
    ...
    while (idx < csize) {
        ...
        if (ret == -1) {
            ...
            log_critical("Warning: syntax error in server data");
            exit(1);
        }
        idx += ret;
    }
