...
int char_moveto(int cn, int x, int y, int flag, int x2, int y2)
{
	...
	case DX_RIGHT:
		...
		if ((in = map[(ch[cn].x + 1) + (ch[cn].y) * MAPX].it)!=0 && !it[in].active && ( it[in].driver==2 || it[in].driver==77 || it[in].driver==78 || it[in].driver==94 ))
		{
			act_use(cn);
			return 0;
		}
	...
	case DX_DOWN:
		...
		if ((in = map[(ch[cn].x) + (ch[cn].y + 1) * MAPX].it)!=0 && !it[in].active && ( it[in].driver==2 || it[in].driver==77 || it[in].driver==78 || it[in].driver==94 ))
		{
			act_use(cn);
			return 0;
		}
