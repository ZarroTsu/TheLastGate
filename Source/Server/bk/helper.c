...
{	// [in] is talisman, [in2] is the target item
	...
	switch (it[in].data[0])
	{
		...
		case 30: it[in2].cool_bonus[0] += 2*mul; break;
...
void set_spawn_talks(int cn)
{
	switch (RANDOM(5))	// Killed our enemy
	{
		...
		case  4:	strcpy(ch[cn].text[0], "Come back when you're stronger!"); break;
		default:	strcpy(ch[cn].text[0], "Yes! Die, %s!"); break;
	}
	switch (RANDOM(5))	// Began fighting/engaging
	{
		...
		case  4:	strcpy(ch[cn].text[1], "Finally, something to kill!"); break;
		default:	strcpy(ch[cn].text[1], "Yahoo! An enemy! Prepare to die, %s!"); break;
	}
	switch (RANDOM(5))	// Killed by player
	{
		...
		case  4:	strcpy(ch[cn].text[3], "Ah, I see now..."); break;
