	...
	*/
	...
	if (!buff)
	{
		...
		if (cn==co && !(ch[cn].flags & CF_AREA_OFF) && (B_SK(cn, SK_PROX) && ((IS_BRAVER(cn) && in==SK_WEAKEN) || 
			(IS_ARCHHARAKIM(cn) && in==SK_BLAST) ||	(IS_SORCERER(cn) && (in==SK_CURSE||in==SK_SLOW||in==SK_POISON)) )) || 
	...
	if (d20!=-1 && (in!=SK_CLEAVE && in!=SK_LEAP && in!=SK_TAUNT && in!=SK_BLIND && in!=SK_DOUSE &&
		...
		(d20 && cn!=co && chance_base(cn, co, in, d20, get_target_resistance(cn, co), usemana)))))
	{
		...
		/*
		// Book: Shiva's Malice :: Curse tries Slow afterward on failure
		if (in==SK_CURSE && get_book(cn, IT_BOOK_SHIV))
		{
			skill_slow(cn, 0);
		}
		// Book: Bishop's Manafesto :: Cast Enhance and Protect after Bless after failure
		if (in==SK_BLESS && get_book(cn, IT_BOOK_BISH))
		{
			skill_protect(cn, 0);
			skill_enhance(cn, 0);
		}
...
int spellcost(int cn, int cost, int in, int usemana)
{
	...
	if (get_gear(cn, IT_TW_CROWN) || get_enchantment(cn, 22)) // These are the same slot, so no harm done merging them
	{
		cost -= cost*20/100;
	}
	...
	if (roll > chance || defense > power + (power / 2))
	{
		...
		if (usemana)
		{
			if (IS_PLAYER(cn) && !IS_PLAYER(co) && !(ch[cn].flags & CF_OVERRIDE))
			{
				...
				ch[co].spellfail = 1;
				do_char_log(cn, 5, 
				"Your spell was suppressed by your target.\n");
				return 0;
			}
			else
			{
...
void add_exhaust(int cn, int len)
{
	...
	if (IS_PLAYER(cn) && ch[cn].spellfail)
	{
		len = len/2;
		ch[cn].spellfail = 0;
	}
...
int spell_curse(int cn, int co, int power, int flag)
{
	...
	// Spell suppression
	if (!IS_PLAYER(co) && ch[co].spellfail) 
	{
...
void skill_blast(int cn)
{
	...
	if (can_aoe)
	{
		if (cast_aoe_spell(cn, co, SK_BLAST, power, aoe_power, cost, count, hit, avgdmg, dr1, dr2) < 0)
			return;
	...
	{
		surround_cast(cn, co_orig, 0, SK_BLAST, power, dr1, dr2);
	}
...
{
	int in, co, n, m, base_pow, power, ail_pow = 0, success = 0, chanc, flag, verd;
...
void skill_ghost(int cn)
{
	...
	int archgc = 0, archbonus = 0, archtmp = 0, dreadplate = 0, necronomicon = 0, invidia = 0, hermit = 0;
	...
	if (IS_PLAYER(cn) && (co = ch[cn].data[PCD_COMPANION]))
	{
		...
		{
			if ((cz = ch[cn].skill_target1) && do_char_can_see(cn, cz))
			{
	...
	if (get_tarot(cn, IT_CH_HERMIT_R)) hermit = 1;
	...
	if (hermit)
	{
		ch[cc].weapon_bonus = ch[cc].weapon_bonus * 5/4;
...
{
	int hitpower, aggravate=0, tmp, tmpmp=0;
...
{
	int power, cost, tmp, tmpmp, co, dam, aggravate = 0;
...
int spell_immolate(int cn, int co, int power, int flag)
{
	int in;
...
void skill_leap(int cn)
{
	...
	power = skill_multiplier(power, cn);
	aoepower = 8 + ch[cn].aoe_bonus;
	critical = ch[cn].crit_multi;
	
	if (critical > 175)
	{
		critical = critical - 175;
		critical = 175 + critical/2;
	}
	if (critical > 250)
	{
		critical = critical - 250;
		critical = 250 + critical/2;
	}
	...
	if (randomtarg)
	{
		...
		for (x = xf; x<xt; x++) for (y = yf; y<yt; y++) 
		{
			...
			{
				if (!do_surround_check(cn, co, 0) || !do_char_can_see(cn, co)) 
				{
					continue;
				}
				catalog[c] = co;
	...
	if (map[m + md].to_ch || (map[m + md].flags & MF_MOVEBLOCK) ||
	   (IS_MONSTER(cn) && (map[m + md].flags & MF_NOMONST)) ||
	   (!IS_MONSTER(cn) && (map[m + md].flags & MF_NOPLAYER)) ||
	   (map[m + md].it && (it[map[m + md].it].flags & IF_MOVEBLOCK)))
	{
...
int spell_zephyr(int cn, int co, int power, int flag)
{
	...
	if (flag)	// Debuff version
	{
		...
		if (IS_SEYAN_DU(cn)) power = power * 5/4;
	...
	else		// Buff version
	{
		...
		if (deathR)
			bu[in].skill[SK_RESIST][1] = power/8;
		else
			bu[in].skill[SK_IMMUN][1] = power/8;
...
int spell_rage(int cn, int co, int power)
{
	...
	if (get_tarot(co, IT_CH_HERMIT_R))
		bu[in].top_damage[1] = power*6/15 + 6;
	else
		bu[in].weapon[1]	 = power*2/15 + 2;
